...
class JsonOutput:
    ...
    def __init__(self, output_dir: str) -> None:
        ...
        # Clean previous run data to avoid accumulation
        for p in (self._endpoints_path, self._secrets_path):
            if p.exists():
                p.unlink()

    async def write_endpoint(self, endpoint: Endpoint) -> None:
        async with aiofiles.open(self._endpoints_path, "a", encoding="utf-8") as f:
            await f.write(endpoint.model_dump_json() + "\n")

    async def write_secret(self, secret: Secret) -> None:
        async with aiofiles.open(self._secrets_path, "a", encoding="utf-8") as f:
